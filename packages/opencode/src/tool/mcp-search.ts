...
async function call(server: string, tool: string, args: Record<string, unknown>, ctx: Tool.Context) {
  ...
  await Plugin.trigger("tool.execute.after", { tool: key, sessionID: ctx.sessionID, callID: ctx.callID, args }, result)

  const processed = processMcpResult(result)

  return {
    title: `${server}/${tool}`,
    output: processed.output || "Success (no output)",
    metadata: { ...processed.metadata, server, tool },
